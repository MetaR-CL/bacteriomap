// AdminScreen.jsx — Admin panel (Supabase for bacteria/systems/zones, localStorage for quiz/meta)
...
function QuizEditor() {
  ...
  const allBact = React.useMemo(() => {
    const seen = new Set();
    const out = [];
    [...LCR_PATHO, ...ORL_PATHO, ...ORL_FLORA].forEach(b => { if (seen.has(b.name)) return; seen.add(b.name); out.push(b); });
    ...
    <div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(3, 1fr)', gap:10 }}>
        {allBact.map(b => {
          ...
          return (
            <label key={b.name} style={{ display:'flex', alignItems:'center', gap:10, padding:'10px 12px', background:T.paper, border:`0.5px solid ${T.rule}`, cursor:'pointer', borderLeft:`3px solid ${on?c.stroke:T.ruleSoft}`, opacity:on?1:0.55 }}>
