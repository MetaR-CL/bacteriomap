// QuizScreen.jsx — Quiz « Qui suis-je ? » avec indices progressifs
import React from 'react';
import { T, ORL_PATHO } from './data.js';
import { LCR_PATHO, MorphoSVG, gramColor } from './shared.jsx';
...
export default function QuizScreen({ navigate }) {
  ...
  const filtered = React.useMemo(() => {
    return pool.filter(b => {
      ...
      if (filterSystem !== 'all') {
        const inORL = ORL_PATHO.some(p => p.name === b.name);
  ...
  const c = gramColor(current.gram);
