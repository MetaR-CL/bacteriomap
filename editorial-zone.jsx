// editorial-zone.jsx — Vue chapitre + sous-zones + planche de bactéries

function ZoneScreen({ navigate, systemId = 'orl', vivid = false, showImages = true }) {
  const sys = SYSTEMS.find(s => s.id === systemId) || SYSTEMS[2];
  const palette = window.getSystemPalette(sys.id);
            ...
            return (
              <div key={s.id} onClick={()=>setActiveSub(s.id)} style={{
                ...
                <div>
                  ...
                  {active && (
                    <div style={{ marginTop:8, fontFamily:T.sans, fontSize:12, color:T.ink3, fontStyle:'italic', lineHeight:1.4, fontFamily:T.serif }}>
                      {s.descr}
                    </div>
                  )}
                </div>
                ...
                return (
                  <figure key={i}
                    ...
                    <div style={{ background:T.bgSoft, height:148, position:'relative', border:`0.5px solid ${T.rule}`, overflow:'hidden' }}>
                      <div style={{ position:'absolute', top:8, left:10, fontFamily:T.mono, fontSize:9, color:T.ink3, zIndex:2, mixBlendMode:'difference', color:'#888' }}>fig. {fig}</div>
